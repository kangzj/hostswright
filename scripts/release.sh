#!/bin/zsh
# Builds a Release Hostswright.app, signs it, packages a DMG, and optionally notarizes it.
#
...
#   scripts/release.sh --identity "..." --team TEAMID --notarize-profile hostswright
#
# The notarization profile is created once with:
#   xcrun notarytool store-credentials hostswright --apple-id you@example.com --team-id TEAMID --password app-specific-password
set -euo pipefail
cd "$(dirname "$0")/.."
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --identity) export HOSTSWRIGHT_SIGNING_IDENTITY="$2"; shift 2 ;;
    --team) export HOSTSWRIGHT_SIGNING_TEAM="$2"; shift 2 ;;
...
xcodebuild -project Hostswright.xcodeproj -scheme Hostswright -configuration Release -derivedDataPath build/Release \
...
app="build/Release/Build/Products/Release/Hostswright.app"
...
dmg="dist/Hostswright-$version.dmg"
hdiutil create -volname "Hostswright" -srcfolder dist/stage -ov -format UDZO "$dmg" >/dev/null
rm -rf dist/stage
